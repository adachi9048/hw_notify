...
// 一对一推送关键词，多个使用&连接
let notify_only_text = '';
if (process.env.NOTIFY_ONLYBYONE) {
    notify_only_text = process.env.NOTIFY_ONLYBYONE;
}
            ...
            if (obs.postFlag == 1) {
                for ( let i = 0 ; i <= obs.maxIndex ; i++ ) {
                    if (obs.uid[i] !== " ") {
                        ...
                        desp = `<font size="3">${text}\n\n${obs.separation[i]}</font>`;
                        if ( ifAuthor.test(desp) === false ) {
                            desp = `<font size="3">${text}\n\n${obs.separation[i]}\n\n${end_txt}</font>`;
                        }
                        desp = desp.replace(/[\n\r]/g, '<br>'); // 默认为html, 不支持plaintext
                        let body = {
                        ...
                        $.post(options, (err, resp, data) => {
                            try {
                                if (err) {
                                    console.log("\nWxPusher发送账号通知调用 API 失败！\n");
                                    console.log(err);
                                } else {
                                    data = JSON.parse(data);
                                    if (data.code === 1000) {
                                        if (i == obs.maxIndex){
                                            console.log("WxPusher发送一对一账号通知消息成功🎉");
                                        }
                                    }
                                }
                            } catch (e) {
                ...
                console.log('未检测到关键词或未匹配到 UID ，取消WxPusher推送消息通知🚫');
                resolve();
            }
        } else {
            console.log('您未提供WxPusher推送所需的 WP_APP_TOKEN 和 WP_UIDS ，取消WxPusher推送消息通知🚫');
            resolve();
        }
    });
}

function strSeparate(text,desp) {
    let separation = [];
    let uid = [];
    let maxIndex = -1;
    let postFlag = 0;
    let crop_DsepFLag = RegExp("已可领取|已成熟"); //判断是否为萌宠和农场领取
    let crop_NickName = RegExp("【京东账号(.*)");
    let crop_DsepSplit = RegExp("(?=【京东账号(.*)】)");
    let crop_NameSplit = RegExp("】(.*)");
    let expire_DespFlag = RegExp("重新登录");    //判断是否为CK失效
    let expire_NickName;
    let expire_NameSplit;
    let beanChange_DespFlag = RegExp("京东资产变动通知");   //判断是否为资产变动
    let beanChange_NickName = RegExp("账号名称(.*)");
    let beanChange_DespSplit = RegExp("📣=============账号(.*)=============📣");
    let beanChange_NameSplit = RegExp("：(.*)");
    let keyWord_SendNotify = RegExp(notify_only_text);
    let keyWord_NameSplit;
    let keyWord_NickName;
    let keyWord_DespSplit;
    if ( beanChange_DespFlag.test(text) == true && keyWord_SendNotify.test(text) == true ) {
        keyWord_NameSplit = beanChange_NameSplit;
        keyWord_NickName = beanChange_NickName;
        keyWord_DespSplit = beanChange_DespSplit;
        postFlag = 1;
    } else if ( crop_DsepFLag.test(desp) == true && keyWord_SendNotify.test(desp) == true ) {
        keyWord_NameSplit = crop_NameSplit;
        keyWord_NickName = crop_NickName;
        keyWord_DespSplit = crop_DsepSplit;
        postFlag = 1;
    } else if ( expire_DespFlag.test(desp) == true && keyWord_SendNotify.test(desp) == true ) {
        keyWord_NameSplit = expire_NameSplit;
        keyWord_NickName = expire_NickName;
        keyWord_DespSplit = expire_NameSplit;
        postFlag = 1;
    }
    if (postFlag) {
        for (let i of desp.split(keyWord_DespSplit)) {  
            if ( i.length !== 0 && keyWord_NickName.test(i) === true) { 
                separation.push(i);
            }
        }
        for ( let i=0 ; i < separation.length ; i++ ) {
            let nicKname = keyWord_NameSplit.exec( separation[i].match(keyWord_NickName)[0] )[1]; 
            let temp = getUids(nicKname);
            uid[i] = temp;
            if ( temp !== " " ) {
                maxIndex = i;
            }
